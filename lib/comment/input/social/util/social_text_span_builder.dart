...
/*
Maybe needed in the furture, not now.

SpanBuilder for highlight regex patterns suchas: mention, hashtag, url, phone, email....
*/
...
class SocialTextSpanBuilder {
  ...
  MatchSearchResult getTextStyleForRange(int start, int end,
    ...
    TextStyle? textStyle;
    DetectedType detectedType = DetectedType.plainText;
    String text = "";
    ...
    for (var type in allMatches.keys) {
      ...
      if (index != -1) {
        ...
        if (isIgnored) {
          ...
          detectedType = DetectedType.plainText;
        } else {
...
///[type] detected type. returns [DetectedType.plainText] by default.
